...
TEST_P(RadixGlobalBaseTest, Histogramv1GlobalReducev1) {
          ...
          ::sort::gpu::radix::details::v1::kernel_local_histogram_v1<256>
                    << <numBlocks, 256 >> > (gpu_array.data(), N, d_local.data().get(), numBlocks * 256,
                              /*rshift=*/0);
...
TEST_P(RadixGlobalBaseTest, Histogramv1GlobalReducev2) {
          ...
          cpu_global_base_ref(std::vector<uint32_t>(gpu_array.begin(), gpu_array.end()), cpu_base);
...
TEST_P(RadixGlobalBaseTest, Histogramv1GlobalReducev3) {
          ...
          const std::size_t pitch = std::max<std::size_t>(1, (numBlocks + 1024 - 1) / 1024); 
         ...
         // ---- round 0: localT -> write_ptr ----
          ::sort::gpu::radix::details::v3::kernel_global_reduce_from_local_v3<32> << <grid, block >> > (
                    localT_ptr,
                    write_ptr,
                    in_stride, in_real,
                    out_stride, out_real);
...
TEST_P(RadixGlobalBaseTest, Histogramv2GlobalReducev1) {
          ...
          ::sort::gpu::radix::details::v2::kernel_local_histogram_v2<BinSize> << <numBlocks, BinSize >> > (
                    gpu_array.data(), N, d_local.data().get(), numBlocks * BinSize, 0);
