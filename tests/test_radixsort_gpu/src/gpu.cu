...
void gpu_local_count_func(uint32_t* d_in, size_t N, std::vector<uint32_t>& out_local_count) {
          ...
          ::sort::gpu::radix::details::v1::kernel_local_histogram_v1<256>
                    << <numBlocks, 256 >> > (
                              d_in, N,
                              d_local.data().get(), numBlocks * BinSize,
                              /*rshift=*/0
                              );
...
void gpu_global_base(uint32_t* d_in, size_t N, std::vector<uint32_t>& out_base) {
          ...
          ::sort::gpu::radix::details::v1::kernel_local_histogram_v1<256> << <numBlocks, 256 >> > (
                    d_in, N,
                    d_local.data().get(), numBlocks * 256,
                    /*rshift=*/0
                    );
          ...
          ::sort::gpu::radix::details::v1::kernel_global_reduce_from_local_v1<256> << <numBlocks, 256 >> > (
                    d_local.data().get(), numBlocks * 256,
                    d_global.data().get()
                    );
