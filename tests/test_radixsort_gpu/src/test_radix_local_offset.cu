...
TEST_P(RadixLocalOffsetTest, GlobalLocalOffsetCPU) {
          ...
          cpu_local_count_ref(
                    std::vector<uint32_t>(gpu_array.begin(), gpu_array.end()),
                    numBlocks,
                    cpu_local
          );
          ...
          gpu_local_offset_func<BlockSize>(
                    gpu_array.data(),
                    N,
                    gpu_local
          );
          ...
          for (size_t b = 0; b < numBlocks; ++b) {
                    for (size_t bin = 0; bin < BinSize; ++bin) {
                              ASSERT_EQ(
                                        cpu_local[b * BinSize + bin],
                                        gpu_local[b * BinSize + bin]
                              ) << "Mismatch at N=" << N
                                        << " block=" << b
                                        << " bin=" << bin;
                    }
