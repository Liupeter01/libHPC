...
TEST_P(RadixLocalOffsetTest, GlobalLocalHierarchicalScanV1) {
          ...
          cudahelper::kernel_clear_u32 << <(d_global.size() + fan_in - 1) / fan_in, fan_in >> > (
                    d_global.data().get(), d_global.size());

          cudahelper::kernel_clear_u32 << <(d_local.size() + fan_in - 1) / fan_in, fan_in >> > (
                    d_local.data().get(), d_local.size());

          cudahelper::kernel_clear_u32 << <(d_localT.size() + fan_in - 1) / fan_in, fan_in >> > (
