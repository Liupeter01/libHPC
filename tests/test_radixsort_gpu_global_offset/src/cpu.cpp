...
void cpu_global_base_ref(const std::vector<uint32_t> &in,
...
void cpu_local_count_ref(const std::vector<uint32_t> &in, size_t numBlocks,
...
void cpu_local_offset_ref(std::vector<uint32_t> &local, // in-place
                          size_t numBlocks) {
  constexpr size_t BinSize = 256;
