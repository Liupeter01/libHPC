          ...
          namespace radix {
                    namespace details {
                              struct helper{
                                        ...
                                        static constexpr inline std::intptr_t sort::radix::details::helper::constexpr_log2(std::intptr_t n) {
                                                  return (n < 2) ? 0 : 1 + constexpr_log2(n >> 1);
                                        }

                                        static void generate_random(std::vector<uint32_t>& vec, const std::size_t numbers);
                              };


#define RADIX_REQUIRE_ALLOWED_BINSIZE(B) do{ \
    static_assert((B ) == 16u  || (B ) == 256u || (B) == 65536u || (B) == 4294967296ull, \
                  "BinSize must be one of {0xF, 0xFF, 0xFFFF, 0xFFFFFFFF}"); \
                              ...
                              static void radix_sort_v1(std::uint32_t* a, std::size_t n) {
                                        ...
                                        static constexpr std::size_t RSHIFT = details::helper::constexpr_log2(BinSize);
                                        static constexpr std::size_t ROUND = std::numeric_limits<uint32_t>::digits / RSHIFT;
...
#pragma omp parallel for schedule(static, 1)
                    ...
                    template <std::size_t BinSize = 256, typename std::enable_if<
                              (BinSize == 16u ||
                                        BinSize == 256u ||
                                        BinSize == 65536u ||
                                        BinSize == 4294967296ull), int>::type = 0>
