...
// -----------------------------------------
// We scan in a hierarchy where each CTA processes exactly one tile (1024
// elems):
//
...
//   - round k: scan each tile of level(k-1) in-place, and emit tile SUMs into
//   level(k)
//
...
//   - Only levels that were used as "in" in some round got tile-local exclusive
//   scan;
...
//     This converts level[k-1] from "tile sums" into "tile offsets" at that
//     level.
//
// Final apply:
...
//   - the highest level must be exclusive-scanned into OFFSETS (or handle
//   level0-only case).
// ================================================================
...
std::vector<LevelDesc> build_level_layout_and_allocate(
    thrust::device_vector<uint32_t> &big_buffer, uint32_t BinSize,
    uint32_t local_real,
...
void hierarchical_exclusive_scan_localT_1024(
    uint32_t *localT, thrust::device_vector<uint32_t> &level_buffer,
    const std::vector<LevelDesc> &levels, uint32_t BinSize,
...
__global__ void kernel_scan_tile1024_exclusive_inplace_and_emit_tile_sum(
  ...
  const uint32_t linear =
      tile * (BlockSize * BlockSize) + warp_id * BlockSize + lane;
...
#pragma unroll
  ...
  // -----------------------------
  // emit tile sum
  //     total tile sum = sum of all 1024 elems = warp_prefix[last] +
  //     warp_sums[last] warp_id==0 && lane==0 does it once
  // -----------------------------
  if (warp_id == 0 && lane == 0) {
...
apply_tile_offset_1024(uint32_t *__restrict dst,       // [bin][dst_stride]
...
__global__ void kernel_apply_tile_offsets_to_element_offsets_1024(
    ...
    uint32_t dst_stride, uint32_t dst_real, uint32_t src_stride,
    uint32_t src_real) {
  apply_tile_offset_1024<BlockSize>(dst_offsets, src_offsets, dst_stride,
                                    dst_real, src_stride, src_real);
}
...
__global__ void kernel_apply_level0_to_localT_1024(
    uint32_t *__restrict localT, const uint32_t *__restrict level0,
    uint32_t local_stride, uint32_t local_real, uint32_t level0_stride,
    uint32_t level0_real) {
  apply_tile_offset_1024<BlockSize>(localT, level0, local_stride, local_real,
                                    level0_stride, level0_real);
}
