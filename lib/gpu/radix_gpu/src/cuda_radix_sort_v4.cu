...
static inline void radix_sort_v4(
    std::vector<uint32_t, CudaAllocator<uint32_t, CudaMemManaged>> &input) {
          ...
          for (std::size_t round = 0; round < round_number; ++round) {
                    ...
                    cudahelper::kernel_clear_u32 << <(global.size() + fan_in - 1) / fan_in, fan_in >> > (
                              global.data().get(), global.size());

                    cudahelper::kernel_clear_u32 << <(local.size() + fan_in - 1) / fan_in, fan_in >> > (
                              local.data().get(), local.size());

                    cudahelper::kernel_clear_u32 << <(localT.size() + fan_in - 1) / fan_in, fan_in >> > (
                              localT.data().get(), localT.size());

                    cudahelper::kernel_clear_u32 << <(reduce_a.size() + fan_in - 1) / fan_in, fan_in >> > (
                              reduce_a.data().get(), reduce_a.size());

                    cudahelper::kernel_clear_u32 << <(reduce_b.size() + fan_in - 1) / fan_in, fan_in >> > (
                              reduce_b.data().get(), reduce_b.size());

                    cudahelper::kernel_clear_u32 << <(big_buffer.size() + fan_in - 1) / fan_in, fan_in >> > (
                    ...
                    */
                    ...
                    // Copy 2 Global
                    cudaMemcpy(global.data().get(), final_result, global.size() * sizeof(uint32_t),
