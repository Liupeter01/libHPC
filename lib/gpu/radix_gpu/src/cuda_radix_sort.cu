                    ...
                    namespace radix {
                              namespace details {
                                        ...
                                        void __kernel_startup() {
                                                  int grid, int block;
                                                  cudaOccupancyMaxPotentialBlockSize(&grid, &block, kernel_startup);
                                                  kernel_startup << <grid, block >> > ();
                                        }
                                        ...
                                        namespace v1 {
                                                  ...
                                                  static inline void radix_sort_v1(std::vector<uint32_t, CudaAllocator<uint32_t, CudaMemManaged>>& input) {
                                                            ...
                                                            static_assert(std::is_unsigned_v<uint32_t >, "radix_sort expects unsigned integers");
                                                            ...
                                                            //The global array base is primarily used to record the total size of each bin in the range[0, BinSize).
                                                               //  This global array is obtained by merging the per - block local histograms, and by applying a prefix sum,
                                                               //            we can compute the base offset of each bin in the global output array(i.e., the global base). 
                                                            ...
                                                            for (std::size_t round = 0; round < round_number; ++round) {
                                                                      ...
                                                                      kernel_local_histogram_v1 < BinSize>
                                                                                << <numBlocks, BinSize >> > (
                                                                                          ping, original_n,
                                                                                          local.data().get(), padded_n,
                                                                                          bits_per_round * round
                                                                                          );
                                                                      nvtxRangePop();
                                                                      ...
                                                                      //Scanning Global
                                                                      thrust::exclusive_scan(thrust::cuda::par.on(0), global.begin(), global.end(), global.begin());
                                                                      ...
                                                                      ::gpu::util::kernel_transpose<uint32_t, BlockSize>
                                                                                << < grid, block >> > (
                                                                                          /*out = */localT.data().get(),
                                                                                          /*in = */local.data().get(),
                                                                                          BinSize,
                                                                      ...
                                                                      dim3 grid_bt(
                                                                                (numBlocks + BlockSize - 1) / BlockSize, // width = numBlocks
                                                                      ...
                                                                      kernel_radix_scatter_v1< BinSize>
                                                                                << <numBlocks, BinSize >> > (
                                                                                          ping, pong, padded_n,
                                                                                          local.data().get(), padded_n,
                                                                                          global.data().get(), BinSize,
                                                                                          bits_per_round * round
                                                                                          );
                                                            ...
                                                            if (ping != input.data()) {
                                                                      cudahelper::checkCuda(cudaMemcpy(input.data(), ping,
                                                                                original_n * sizeof(uint32_t), cudaMemcpyDeviceToHost));
                                                            }
