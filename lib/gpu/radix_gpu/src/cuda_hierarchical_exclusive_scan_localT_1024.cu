...
void hierarchical_exclusive_scan_localT_1024(
  ...
  // -----------------------------
  {
    ...
    v1::kernel_scan_tile1024_exclusive_inplace_and_emit_tile_sum<32>
        <<<grid, block, 0, stream>>>(localT, level0_ptr, local_stride,
                                     local_real, levels[0].stride, level0_real);
    ...
    for (std::size_t k = 1; k < levels.size(); ++k) {
      ...
      v1::kernel_scan_tile1024_exclusive_inplace_and_emit_tile_sum<32>
          <<<grid, block, 0, stream>>>(Lbase + in.base, Lbase + out.base,
                                       in.stride, in.real, out.stride,
                                       out.real);
    }
  }
  ...
  // -----------------------------
  {
    ...
    v1::kernel_exclusive_scan_upto1024_per_bin<32>
        <<<grid, blk, 0, stream>>>(Lbase + top.base, top.stride, top.real);
  }
  ...
  if (levels.size() >= 2) {
    for (std::size_t k = levels.size(); k-- > 1;) {
      ...
      v1::kernel_apply_tile_offsets_to_element_offsets_1024<32>
          <<<grid, block, 0, stream>>>(Lbase + lo.base, Lbase + hi.base,
                                       lo.stride, lo.real, hi.stride, hi.real);
    }
  }
...

  {
    ...
    v1::kernel_apply_level0_to_localT_1024<32><<<grid, block, 0, stream>>>(
        localT, Lbase + levels[0].base, local_stride, local_real,
        levels[0].stride, level0_real);
  }
}
