...
static inline void radix_sort_v2(
  ...
  for (std::size_t round = 0; round < round_number; ++round) {
    ...
    kernel_local_histogram_v2<BinSize><<<numBlocks, BinSize>>>(
    ...
    kernel_global_reduce_from_local_v2<BinSize><<<BinSize, 32>>>(
        localT.data().get(), padded_n, global.data().get(), numBlocks);
    nvtxRangePop();
    ...
    kernel_radix_scatter_v2<BinSize><<<numBlocks, BinSize>>>(
