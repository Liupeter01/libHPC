...
void __benchmark_dim3() {
  ...
  parallel_transpose_dim3<<<dim3(nx / 32, ny / 32, 1), dim3(32, 32, 1)>>>(
...
void __benchmark_dim3_shared() {
  ...
  parallel_transpose_shared<int, 32>
      <<<dim3(nx / 32, ny / 32, 1), dim3(32, 32, 1)>>>(out.data(), in.data(),
                                                       nx, ny);
  nvtxRangePop();
...
void __benchmark_dim3_shared_solv_conflict() {
  ...
  parallel_transpose_solv_conflict<int, 32>
      <<<dim3(nx / 32, ny / 32, 1), dim3(32, 32, 1)>>>(out.data(), in.data(),
                                                       nx, ny);
  nvtxRangePop();
...
void __benchmark_all() {
...

  {
    ...
    printf("Current Test is: %s\n", "parallel_transpose_baseline");
    nvtxRangePushA("parallel_transpose_baseline");
    parallel_transpose<<<(nx * ny) / 1024, 1024>>>(out.data(), in.data(), nx,
                                                   ny);
    nvtxRangePop();
