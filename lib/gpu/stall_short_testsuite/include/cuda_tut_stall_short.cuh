...
template<typename _Ty>
__global__ void parallel_transpose_dim3(_Ty* out, const _Ty* in, int nx, int ny) {
...
__global__ void parallel_transpose_shared(_Ty* out, const _Ty* in, int nx, int ny) {
...
template<typename _Ty, std::size_t BlockSize>
__global__ void parallel_transpose_solv_conflict(_Ty* out, const _Ty* in, int nx, int ny) {
		  int x = blockIdx.x * BlockSize + threadIdx.x;
		  int y = blockIdx.y * BlockSize + threadIdx.y;
		  if (x >= nx || y >= ny) return;
		  int rx = blockIdx.y * BlockSize + threadIdx.x;
