...
void __benchmark_overlap_pipeline_sweep(const int nStreams) {
  ...
  for (int i = 0; i < nStreams; ++i) {
    int off = i * chunk;
    cudaMemcpyAsync(&d[off], &h[off], chunkBytes, cudaMemcpyHostToDevice,
                    streams[i]);
    simple_kernel<<<chunk / blockSize, blockSize, 0, streams[i]>>>(d, off);
    cudaMemcpyAsync(&h[off], &d[off], chunkBytes, cudaMemcpyDeviceToHost,
                    streams[i]);
  }
  ...
  for (int i = 0; i < nStreams; ++i)
    cudaMemcpyAsync(&d[i * chunk], &h[i * chunk], chunkBytes,
                    cudaMemcpyHostToDevice, streams[i]);

  for (int i = 0; i < nStreams; ++i)
    simple_kernel<<<chunk / blockSize, blockSize, 0, streams[i]>>>(d,
                                                                   i * chunk);

  for (int i = 0; i < nStreams; ++i)
    cudaMemcpyAsync(&h[i * chunk], &d[i * chunk], chunkBytes,
