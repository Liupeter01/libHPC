          ...
          */
          struct alignas(16) ref_counter_packed {
                    ...
                    bool both_zero() const {
                              packed_t val = counter.load(std::memory_order_acquire);
                              return (!(val >> THREAD_REF_SHIFT) && !(val & HEAD_TAIL_MASK));
                    }

                    int get_threads_ref() const {
                              return static_cast<int>(counter.load(std::memory_order_acquire) >> THREAD_REF_SHIFT);
                    }

                    int get_head_tail_ref() const {
                              return static_cast<int>(counter.load(std::memory_order_acquire) & HEAD_TAIL_MASK);
                    }
