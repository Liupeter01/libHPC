...
template <typename _Ty> class QueueLk;

template <typename _Ty> class ConcurrentQueueLk;
} // namespace concurrency

template <typename _Ty> class concurrency::QueueLk {
public:
  ...
  void push(_Ty &&value) {
    auto ptr = std::make_shared<_Ty>(std::move(value));
    {
      std::lock_guard<std::mutex> _lckg(m_mtx);
      m_data.push(ptr);
    }
...
template <typename _Ty> class concurrency::ConcurrentQueueLk {
private:
  struct Node {
