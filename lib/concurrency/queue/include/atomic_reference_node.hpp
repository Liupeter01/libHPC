...
namespace concurrency {
          ...
          struct Node {
                    ...
                    void release_curr_thread_ref() {
                              inner_ref_counter.dec_thread_ref();
                    }
...
#error "Unsupported pointer size"
                    ...
                    static std::intptr_t extract_ref(packed_t val) {
                              // Sign-extension-safe if ref never goes negative (or if you want to allow negative ref)
                    ...
                    bool compare_exchange_weak(reference_node& expected, const reference_node& desired,
                    ...
                    bool compare_exchange_strong(reference_node& expected, const reference_node& desired,
                              std::memory_order success = std::memory_order_acq_rel,
                              std::memory_order fail = std::memory_order_acquire) {
                              packed_t expected_raw = pack(expected.node, expected.thread_ref_counter);
                              packed_t desired_raw = pack(desired.node, desired.thread_ref_counter);
                    ...
                    std::atomic<packed_t> counter{ static_cast<packed_t>(0) }; // init thread_ref_counter = 0  node = nullptr(0)
          };
}
