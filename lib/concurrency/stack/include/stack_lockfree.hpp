...
#define MAX_HAZARD_NUMBER 128
                    ...
                    class hazard_data{
                              ...
                              template<typename _Ty> struct ReclaimNode {
                                        ReclaimNode(node_pointer <_Ty> m)
                                                  :next(nullptr) 
                                                  , node(m)
                                        {}
                              ...
                              /*reclaim linklist*/
                              template<typename _Ty> 
                              using reclaim_pointer = ReclaimNode<_Ty>*;

                              template<typename _Ty>
                              static std::atomic< reclaim_pointer<_Ty>> reclaim_head;
                    ...
                    public:
                              ...
                              template<typename _Ty>
                              static void add_to_reclaim_list(details::hazard_data::node_pointer<_Ty> node) {
                                        auto* new_node = new  details::hazard_data::reclaim_pointer<_Ty>(node);
                                        details::hazard_data::reclaim_pointer<_Ty> expected = hazard_data::reclaim_head<_Ty>.load();
                                        do {
                                                  new_node->next = expected;
                                        } while (!hazard_data::reclaim_head<_Ty>.compare_exchange_weak(expected, new_node));
                              }
                              ...
                              template<typename _Ty>
                              static void auto_remove_from_reclaim_list() {
                                        //It will not influence the pervious one
                                        details::hazard_data::reclaim_pointer<_Ty> head = hazard_data::reclaim_head<_Ty>.exchange(nullptr);
                                        while (head) {
                    ...
                    protected:
                              [[nodiscard]]  hazard_pointer* find_unoccupancy() {
                                        for (std::size_t index = 0; index < MAX_HAZARD_NUMBER; ++index) {
                                                  ...
                                                  if (hazard_data::hazard_pointers[index].id.compare_exchange_strong(id, std::this_thread::get_id())) {
...
template<typename _Ty>
class concurrency::ConcurrentStack {

          friend class details::hazard_data; 

          struct Node {
                    Node() :data(nullptr), next(nullptr) {}
                    Node(const _Ty& value) : data(std::make_shared<_Ty>(value)), next(nullptr) {}
                    Node(_Ty&& value) : data(std::make_shared<_Ty>(std::move(value))), next(nullptr) {}
...
protected:
          ...
          std::optional<std::shared_ptr<_Ty>>  __pop_by_hazard_ptr() {
                    ...
                    details::hazard_manager::auto_remove_from_reclaim_list();
                    return res;
          }
private:
