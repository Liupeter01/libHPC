...
template<typename _Ty>
class  concurrency::LinkListLK {
          struct Node {
                    Node() :next(nullptr), data(nullptr) 
                    {}
                    Node(const _Ty& value) :next(nullptr), data(std::make_shared<_Ty>(value)) 
                    {}
                    std::mutex mutex;
...
public:
          ...
          template<typename _Pred> void remove_if(_Pred&& pred) {
                    ...
                    while (Node* next = current->next.get()) {
                              ...
                              if (pred(*next->data)) {
                                        auto __ext_life_length = std::move(current->next);        //no longer need this, but wee need it's life length!
          ...
          template<typename _Pred>
          std::optional<std::shared_ptr<_Ty>>
                    find_first_if(_Pred&& pred) {
                    Node* current = m_head.get(); //currently, head node;
                    std::unique_lock<std::mutex> _prev_lckg(m_head->mutex);
                    while (Node* next = current->next.get()) {
                              std::unique_lock<std::mutex> _cur_lckg(next->mutex);
