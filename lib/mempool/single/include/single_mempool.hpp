...
class SingleMemoryPool {
          struct NgxPoolDeletor { void operator()(ngx_pool_s* s); };

public:
          SingleMemoryPool() :size_{}, pool_(nullptr) {}
          SingleMemoryPool(const std::size_t pool_size, const std::size_t alignment = NGX_POOL_ALIGNMENT);
...
public:
          ...
          std::vector<std::shared_ptr<_Ty>>
                    create_batch(const std::size_t number, Args &&... args) {
          ...
          void reset();
          void destroy();
          void resize(const std::size_t pool_size, const std::size_t alignment = NGX_POOL_ALIGNMENT);
...
protected:
          ...
          construct(Args &&... args){
                    ...
                    if (!handle || !handle->data) throw std::bad_alloc();

                    _Ty* raw = details::construct(reinterpret_cast<_Ty*>(handle->data), std::forward<Args>(args)...);
          ...
          std::shared_ptr<_Ty>
           construct(Args &&... args) {
                    ...
                    return std::shared_ptr<_Ty>(raw, [pool = pool_.get(), size = size_]( _Ty* s) {
                              if (s) {
