...
static void *ngx_palloc_small(struct ngx_pool_s*pool, size_t size, unsigned char is_align);
...
struct ngx_pool_s* ngx_create_pool(const size_t size)
{
    ...
    p->max = p->d.end - p->d.last < NGX_MAX_ALLOC_FROM_POOL ?
              p->d.end - p->d.last : NGX_MAX_ALLOC_FROM_POOL;
...
void
ngx_destroy_pool(struct ngx_pool_s *pool)
{
...
void
ngx_reset_pool(struct ngx_pool_s *pool)
{
          struct ngx_pool_s        *p;
...
void *
ngx_palloc(struct ngx_pool_s* pool, size_t size)
{
          if (!size) return NULL;
...
void *
ngx_pnalloc(struct ngx_pool_s* pool, size_t size)
{
          if (!size) return NULL;
...
static void *
ngx_palloc_small(struct ngx_pool_s* pool, size_t size, unsigned char is_align)
{
          if (!size) return NULL;
...
static void *
ngx_palloc_block(struct ngx_pool_s* pool, size_t size)
{
          if (!size) return NULL;
...
static void *
ngx_palloc_large(struct ngx_pool_s* pool, size_t size)
{
          if (!size) return NULL;
...
int
ngx_pfree(struct ngx_pool_s *pool, void *p)
{
    struct ngx_pool_large_s  *l;
...
void *
ngx_pcalloc(struct ngx_pool_s *pool, size_t size)
{
...
ngx_pool_cleanup_add(struct ngx_pool_s *p, size_t size)
{
    struct ngx_pool_cleanup_s *c = ngx_palloc(p, sizeof(struct ngx_pool_cleanup_s));
    if (c == NULL) 
        return NULL;
